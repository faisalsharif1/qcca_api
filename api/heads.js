...
module.exports = (function () {
    ...
    router.post('/api/heads', [body().isArray(),
        body('*.Id', 'Id is necessary').exists(),
        body('*.HeadName', 'HeadName is necessary').exists(),
        ...
        connection.query('CALL heads_Process(?)', [JSON.stringify(req.body)], function (err, results, fields) {
            if (err) {
                res.status(500).json({
