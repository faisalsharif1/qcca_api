...
module.exports = (function () {
    ...
    router.post('/api/bankaccounts', [body().isArray(),
        body('*.Id', 'Id is necessary').exists(),
        body('*.BankId', 'Bank Id is necessary').exists(),
        body('*.AccountTitle', 'Account Title Is necessary').exists(),
        body('*.AccountNumber', 'Account Number Is necessary').exists(),
        body('*.AccountId', 'Account Number Is necessary').exists(),
        ...
        connection.query('CALL bankaccounts_process(?)', [JSON.stringify(req.body)], function (err, results, fields) {
