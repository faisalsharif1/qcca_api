...
module.exports = (function () {
    ...
    router.post('/api/userpermissions', [body().isArray(),
        body('*.Id', 'Id is necessary').exists(),
        body('*.FormName', 'FormName is necessary').exists(),
        body('*.IsAllowed', 'IsAllowed is necessary').exists(),
        ...
        connection.query('CALL userpermissions_Process(?)', [JSON.stringify(req.body)], function (err, results, fields) {
            if (err) {
                res.status(500).json({
